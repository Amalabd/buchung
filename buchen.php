<?php

include_once("verbind.php");

?>

<!DOCTYPE html>
<html lang="en">
<head>
    <meta charset="UTF-8">
    <meta http-equiv="X-UA-Compatible" content="IE=edge">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <script src="https://unpkg.com/vue@3/dist/vue.global.js"></script>
    <link href="https://cdn.jsdelivr.net/npm/bootstrap@5.0.2/dist/css/bootstrap.min.css" 
    rel="stylesheet" integrity="sha384-EVSTQN3/azprG1Anm3QDgpJLIm9Nao0Yz1ztcQTwFspd3yD65VohhpuuCOmLASjC" 
    crossorigin="anonymous">

 <link href="https://cdnjs.cloudflare.com/ajax/libs/font-awesome/6.0.0-beta3/css/all.min.css" rel="stylesheet">
 <link href= "buchen.css" rel="stylesheet">
   

    <title>Buchung</title>
</head>
<body>
<!----------------NavBar----------->
<nav class="navbar navbar-light  shadow naa  ">

    <div class="container-fluid  ">
        <div class="">
        <img src="pic/logo2.png" alt="shap" width="55px" class="">
        <span class="fs-3 fw-bold text-success ">Buchung Seite</span>
      </div>
    </div>
  </nav>
<!----------------------------------------------------------------->
<div id="app">
<div class="container sec shadow text-center mt-3 rounded">

<personal-com v-if="showCom" 
v-model:vor-name="vorName"
v-model:nach-name="nachName"
></personal-com>
<!----------------------------------------------------------------->


<div v-if="showCom" class=" row m-2 col-10  align-items-center ">
    <input class="form-check-input" type="checkbox" @click="toggleComponents"/>
    <buchen-com 
  v-model:begin-date="beginDate" 
  v-model:end-date="endDate" 
  v-model:days-difference="daysDifference" 
  v-model:select="select"
  :total="total"
/>
</div>

<div v-if="showCom" v-for="n in count" :key="n" class="row m-2  align-items-center">
<input class="form-check-input" type="checkbox" @click="toggleComponents"/>
<buchen-com 
  v-model:begin-date="beginDate" 
  v-model:end-date="endDate" 
  v-model:days-difference="daysDifference" 
  v-model:select="select"
  :total="total"
/>
</div>
<bill-com 
      :begin-date="beginDate" 
      :end-date="endDate" 
      :days-difference="daysDifference" 
      :total="total"
      :vor-name="vorName"
      :nach-name="nachName"
    />


</div>
</div>
<script src="./buchen.js"></script>
</body>
</html>